...
from my_idlelib.idle_test.mock_tk import Var
...
class ModuleNameTest(unittest.TestCase):
    ...
    def test_good_module_name(self):
        dialog = self.Dummy_ModuleName('my_idlelib')
...
class ModulenameGuiTest(unittest.TestCase):
    ...
    def test_click_module_name(self):
        root = Tk()
        root.withdraw()
        dialog =  query.ModuleName(root, 'T', 't', 'my_idlelib', _utest=True)
        self.assertEqual(dialog.text0, 'my_idlelib')
        self.assertEqual(dialog.entry.get(), 'my_idlelib')
        dialog.button_ok.invoke()
