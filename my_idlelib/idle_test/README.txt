README FOR IDLE TESTS IN my_idlelib.IDLE_TEST
...
python -m my_idlelib.idle_test.htest


1. Test Files

The idle directory,  my_idlelib, has over 60 xyz.py files. The idle_test
...
if __name__ == "__main__":
    ...
    main('my_idlelib.idle_test.test_abc', verbosity=2, exit=False)
...
GUI tests, in test_xyz.py. (Both '-m  my_idlelib' and '-m my_idlelib.idle'
start Idle and so cannot run tests.)

python -m my_idlelib.xyz
python -m my_idlelib.idle_test.test_xyz

The following runs all idle_test/test_*.py tests interactively.

>>> import unittest
>>> unittest.main('my_idlelib.idle_test', verbosity=2)

The following run all Idle tests at a command line.  Option '-v' is the
same as 'verbosity=2'.

python -m unittest -v my_idlelib.idle_test
python -m test -v -ugui test_idle
python -m test.test_idle

The idle tests are 'discovered' by
 my_idlelib.idle_test.__init__.load_tests, which is also imported into
...
python -m unittest -v my_idlelib.idle_test.test_xyz.Test_case.test_meth
python -m test -m my_idlelib.idle_test.text_xyz.Test_case.test_meth test_idle
...
human verification. They are contained in  my_idlelib/idle_test/htest.py,
...
set src= my_idlelib.%1
if "%2" EQU "" set tst=f:/dev/3x/Lib/ my_idlelib/idle_test/test_%1.py
if "%2" NEQ "" set tst=f:/dev/ex/Lib/ my_idlelib/idle_test/test_%2.py
